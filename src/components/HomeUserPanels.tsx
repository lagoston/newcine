...
function computeLockedTags(
  movieCount: number,
  predCount: number,
  recCount: number,
  followerCount: number
): LockedTag[] {
  const locked: LockedTag[] = [];

  for (const tier of PROGRESSION_TIERS) {
    if (movieCount < tier.min) {
      locked.push({ name: tier.name, emoji: tier.emoji, category: 'Progressão', hint: tier.hint });
    }
  }
  for (const tier of ORACLE_PRED_TIERS) {
    if (predCount < tier.min) {
      locked.push({ name: tier.name, emoji: tier.emoji, category: 'Oráculo', hint: tier.hint });
    }
  }
  for (const tier of ORACLE_REC_TIERS) {
    if (recCount < tier.min) {
      locked.push({ name: tier.name, emoji: tier.emoji, category: 'Oráculo', hint: tier.hint });
    }
  }
  for (const tier of COMMUNITY_TIERS) {
    if (followerCount < tier.min) {
      locked.push({ name: tier.name, emoji: tier.emoji, category: 'Comunidade', hint: tier.hint });
    }
  }
...
const HomeUserPanels: React.FC<Props> = ({ userId, username }) => {
  ...
  const fetchUserStats = useCallback(async () => {
    try {
      const [profileRes, moviesRes, followsRes] = await Promise.all([
        supabase
      ...
      const mCount = moviesRes.count ?? 0;
      setLibraryCount(mCount);
      ...
      if (!tagPickedRef.current) {
        const locked = computeLockedTags(mCount, predCount, recCount, follCount);
        if (locked.length > 0) {
        ...
        <div className="relative z-10 p-6">
          ...
          {loadingMovie ? (
            ...
            <div className="flex gap-4 items-start mb-5">
              <div className="flex-shrink-0 w-[88px] h-[132px] rounded-2xl overflow-hidden shadow-xl border border-white/30 dark:border-gray-700/30">
                <OptimizedPoster
              ...
              <div className="flex-1 min-w-0 pt-1">
                <h4 className="text-base font-bold text-gray-900 dark:text-white leading-snug mb-1.5 line-clamp-2">
                  {dailyMovie.title}
                </h4>
