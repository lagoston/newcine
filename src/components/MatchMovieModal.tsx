            ...
            {!showAddViewer && phase === 'setup' && (
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {MOODS.map(({ labelKey, value, bg, hover, text, border }) => {
                    const isSelected = selectedMoods.includes(value);
                    return (
                      <button
                        key={value}
                        onClick={() => toggleMood(value)}
                        className={`px-3 py-1.5 rounded-full text-xs font-semibold border backdrop-blur-sm transition-all ${bg} ${hover} ${text} ${
