...
DECLARE
  ...
  subscription_rows_affected int;
  result jsonb;
BEGIN
  ...
  GET DIAGNOSTICS subscription_rows_affected = ROW_COUNT;
  RAISE LOG 'Affected % subscription rows for customer: %', subscription_rows_affected, customer_id_input;
...
-- Fix specific customer record for Ikeda if needed
DO $$
DECLARE
  ikeda_user_id uuid := '7e644f43-38ab-44d8-8c46-65df73a490bb';
  ...
  customer_count int;
BEGIN
  -- Check if there's already a record with this combination of user_id and customer_id
  SELECT COUNT(*) INTO customer_count
  FROM stripe_customers
  WHERE (user_id = ikeda_user_id AND customer_id = ikeda_customer_id) 
    OR customer_id = ikeda_customer_id;
  
  IF customer_count > 0 THEN
    -- Update existing record to ensure it has the correct values
    ...
    WHERE user_id = ikeda_user_id OR customer_id = ikeda_customer_id;
    
    RAISE LOG 'Updated existing customer record for Ikeda (user_id: %, customer_id: %)', 
      ikeda_user_id, ikeda_customer_id;
  ELSE
    -- If no record exists at all, it's safe to insert
    BEGIN
      INSERT INTO stripe_customers (user_id, customer_id)
      VALUES (ikeda_user_id, ikeda_customer_id);
      RAISE LOG 'Created new customer record for Ikeda';
    EXCEPTION WHEN unique_violation THEN
      -- If we somehow get a unique violation, handle it gracefully
      RAISE LOG 'Got unique violation when trying to create customer record for Ikeda, will try update instead';
      UPDATE stripe_customers
      SET customer_id = ikeda_customer_id, updated_at = now()
      WHERE user_id = ikeda_user_id;
      RAISE LOG 'Updated customer record for Ikeda as fallback';
    END;
  END IF;
END $$;
