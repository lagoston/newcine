/*
  # Fix calculate_subcategory_result function

  ## Problem
  The function was trying to UPDATE `profiles.antagonistic_subcategory`, a column
  that does not exist. The correct column is `subcategoria_id`.

  ## Changes
  - Recreate `calculate_subcategory_result` using the correct column name `subcategoria_id`
    instead of `antagonistic_subcategory`
*/
...
BEGIN
  ...
  UPDATE profiles
  SET
    subcategoria_id = winner_subcategory,
    updated_at = NOW()
  WHERE id = p_user_id;
