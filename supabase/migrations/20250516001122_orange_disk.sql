/*
  ...
  1. Changes
    ...
    - Fix issue with negative time values being displayed
    - Force regeneration of all reset times for consistency
    - Ensure reset values are always in the future

  2. Purpose
    - Fix the negative time display issue (-21h)
    - Make sure all users see the correct next reset time
    - Create a permanent fix for ticket reset calculation
*/
...
BEGIN
  ...
  FOR rec IN SELECT user_id FROM user_tickets LOOP
    -- Call check_and_reset_tickets for each user to update their next_reset
    PERFORM check_and_reset_tickets(rec.user_id);
  END LOOP;
END $$;